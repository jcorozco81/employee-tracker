...
let departmentList;
const departmentListQuery = () => {db.query('SELECT department_name FROM department', (err, res) => {
  if (err) throw err;
...
let roleList;
const roleListQuery = () => {db.query('SELECT title FROM roles', (err, res) => {
  if (err) throw err;
...
let employeeList;
const employeeListQuery = () => {db.query('SELECT id, first_name, last_name FROM employee', (err, res) => {
  if (err) throw err;
...
// View Records
            const loadEmployeeList = () => {
              // view all Employees
                ...
                db.query('SELECT A.id AS "Employee ID", A.first_name AS "Employee First Name", A.last_name AS "Employee Last Name", roles.title AS "Title", roles.salary AS "Salary", department.department_name AS "Department", B.first_name AS "Manager First Name", B.last_name AS "Manager Last Name" FROM employee A     LEFT JOIN roles ON roles_id=roles.id LEFT JOIN department ON roles.department_id = department.id LEFT JOIN employee B on B.id = A.manager_id;', (err, res) => {
                  if (err) throw err;
                  console.table(res);
                  init();
                });
...
db.query(query_s, [answer.department], (err,res_s) =>{
  ...
  init();
              ...
              const addToEmployeeList = () => {
                ...
                db.query(query, [answer.firstName, answer.lastName, id_query, mn_query], (err,res) =>{
                if (err) throw err;
                console.log(`Records updated`);
                init();
              });
...
// Modify
  ...
  .then((answer) => {
    ...
    const query_en = 'SELECT id FROM employee WHERE first_name = ? AND last_name = ?';
